...
- [x] 4.4 增加可删除重建的 Parquet/CSV 缓存并验证缓存不参与事实判定

## 5. 文档与端到端验证

- [x] 5.1 同步 PRD、统一配置、Schema 文档和 reviewer 指南中的双 profile 与唯一事实源规则
- [x] 5.2 使用 pass、warn、hard fail、runtime error 四类 fixture 验证单资产完整 revision 轨迹
- [x] 5.3 使用同一批输入端到端验证 acceptance 截断与 supplier_evaluation 全流程的差异
- [x] 5.4 运行全量测试并记录旧 sidecar 到 QC JSON 的迁移与回滚说明
