...
def build_review_index_video_html(rows: list[dict[str, Any]]) -> str:
    ...
    return (
        ...
        "function addAffectedSegment(rowIndex){const row=REVIEW_ROWS[rowIndex]; const key=rowKey(row); if(!segmentsByReviewId[key]) segmentsByReviewId[key]=[]; segmentsByReviewId[key].push(makeSegment(row)); renderSegments(rowIndex);}\n"
        "function confirmWholeWindowAffected(rowIndex){const row=REVIEW_ROWS[rowIndex]; const key=rowKey(row); segmentsByReviewId[key]=[makeSegment(row,{affected_start_frame:row.window_start_frame,affected_end_frame:row.window_end_frame,manual_outcome:'true_positive',acceptance_status:'rejected'})]; renderSegments(rowIndex);}\n"
        "function markWholeWindowFalsePositive(rowIndex){const row=REVIEW_ROWS[rowIndex]; const key=rowKey(row); segmentsByReviewId[key]=[makeSegment(row,{segment_id:`${key}_false_positive`,affected_start_frame:'',affected_end_frame:'',manual_outcome:'false_positive',failure_mode:'acceptable_minor_misalignment',severity:'low',confidence:'medium',acceptance_status:'accepted'})]; renderSegments(rowIndex);}\n"
        ...
        "function markWholeWindowNeedsReview(rowIndex){const row=REVIEW_ROWS[rowIndex]; const key=rowKey(row); segmentsByReviewId[key]=[makeSegment(row,{affected_start_frame:row.window_start_frame,affected_end_frame:row.window_end_frame,manual_outcome:'review',acceptance_status:'review'})]; renderSegments(rowIndex);}\n"
        "function deleteSegment(rowIndex,segmentIndex){const row=REVIEW_ROWS[rowIndex]; const key=rowKey(row); (segmentsByReviewId[key] || []).splice(segmentIndex,1); renderSegments(rowIndex);}\n"
        ...
        "function setSegmentFrame(rowIndex,segmentIndex,field){const frame=currentVideoFrame(rowIndex); if(frame === ''){setStatus('Cannot compute current frame without video and fps'); return;} const el=document.getElementById(segmentFieldId(rowIndex,segmentIndex,field)); if(el){el.value=frame;}}\n"
        ...
        "function saveProgress(){localStorage.setItem(STORAGE_KEY,JSON.stringify(captureProgress())); setStatus('Saved progress locally');}\n"
        "function loadProgress(){const raw=localStorage.getItem(STORAGE_KEY); if(!raw){setStatus('No saved progress'); return;} restoreProgress(JSON.parse(raw)); render(); setStatus('Loaded local progress');}\n"
