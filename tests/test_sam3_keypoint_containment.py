...
def test_side_view_manual_review_window_does_not_hard_fail() -> None:
    rows = classify_containment_rows(
        [
            {
                **_frame_row(0.0, frame_idx=1),
                "source_review_type": ["side_view_manual_review"],
                "source_trigger_reason": ["side_view_hand_orientation"],
                "source_needs_manual_review": True,
                "source_sam3_containment_eligible": False,
            },
            {
    ...
    assert summary["strong_fail_frame_count"] == 5
    assert summary["source_review_type"] == ["side_view_manual_review"]
    assert summary["source_sam3_containment_eligible"] is False
    assert summary["window_containment_verdict"] == "side_view_manual_review"
    assert "side-view hand orientation makes SAM3 containment unreliable" in summary["reason"]
